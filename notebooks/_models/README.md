Absolutely—here’s a single, self‑contained `fit_and_forecast.py` you can drop into your repo. It:

* **Scans a single CSV** with multiple stations (or one station),
* **Infers frequency & seasonality** (overridable via CLI),
* **Denoises** each station’s series via **STL** and exports the components,
* **Tries three model families per station** (Seasonal Naive, ETS/Holt‑Winters, SARIMA),
* **Selects the best model by rolling time‑series CV** (sMAPE),
* **Fits the winner on the full series and exports forecasts + 80%/95% intervals**,
* Writes a **per‑station report JSON** plus a **run summary**.

It only depends on **pandas, numpy, statsmodels** (no Prophet/pmdarima).
...
python fit_and_forecast.py \
  ...
  --outdir outputs \
  --freq auto \
  --seasonal-periods auto \
  --impute linear
```

**Key flags**

* `--input`: CSV with at least `[time, station, value]`
* `--time-col`, `--station-col`, `--value-col`: column names
* `--horizon`: forecast steps ahead
* `--outdir`: where artifacts go (per‑station subfolders)
* `--freq`: explicit Pandas frequency (e.g. `H`, `D`, `W`, `M`), or `auto`
* `--seasonal-periods`: integer or `auto` (auto maps common frequencies: H→24, D→7, W→52, M→12, Q→4)
* `--impute`: `none|ffill|bfill|linear` for gaps after aligning to frequency
* `--n-splits`: number of rolling CV splits (default auto)
* `--initial-window`: initial training window for CV (default auto)

---

### What it exports (per station)

* `denoised.csv` — columns: `timestamp, y, trend, seasonal, resid, denoised`
* `forecast.csv` — columns: `timestamp, y_pred, lo_80, hi_80, lo_95, hi_95`
* `model_report.json` — winning model, CV scores, chosen configs, diagnostics

A `run_summary.json` aggregates all stations.
...
def snaive_intervals(y_fit_resid: np.ndarray, h: int, sp: int) -> Tuple[np.ndarray, np.ndarray]:
...
def ets_fit_predict(y_train: pd.Series, h: int, sp: int, config: Dict) -> Tuple[np.ndarray, Optional[np.ndarray]]:
...
def ets_intervals(resid: np.ndarray, h: int, alpha: float) -> Tuple[np.ndarray, np.ndarray]:
...
def sarima_select_order(y: pd.Series, sp: int) -> Optional[Tuple[Tuple[int, int, int], Tuple[int, int, int, int]]]:
    """
...
def sarima_fit_predict(y_train: pd.Series, h: int, order: Tuple[int, int, int],
                       seasonal_order: Tuple[int, int, int, int]) -> Tuple[np.ndarray, Optional[np.ndarray]]:
    with warnings.catch_warnings():
        warnings.simplefilter("ignore")
        mod = SARIMAX(
            y_train,
...
def sarima_intervals(y_train: pd.Series, h: int, order, seasonal_order, alpha: float) -> Tuple[np.ndarray, np.ndarray]:
    with warnings.catch_warnings():
        warnings.simplefilter("ignore")
        mod = SARIMAX(
            y_train,
...
def rolling_cv(
    ...
    sarima_orders: Optional[Tuple[Tuple[int, int, int], Tuple[int, int, int, int]]] = None,
) -> float:
    """
...
def process_station(
    ...
    candidates_summary["ets"] = {"cv_smape": float(best_ets_score), "config": best_ets_cfg}
    ...
    if sarima_orders is not None:
        ...
        candidates_summary["sarima"] = {"cv_smape": float(np.inf), "order": None, "seasonal_order": None}
    ...
    if best_family == "snaive":
        ...
        if len(y) > sp:
            ...
            resid = (y.iloc[-sp:].to_numpy() - snaive_in_sample)  # small sample; fine as heuristic
        else:
            resid = np.array([0.0])
        ...
        lo80_hw, hi80_hw = sarima_intervals(y, horizon, order, seasonal_order, alpha=0.20)
        lo95_hw, hi95_hw = sarima_intervals(y, horizon, order, seasonal_order, alpha=0.05)
        lo80 = yhat - lo80_hw
        hi80 = yhat + hi80_hw
        lo95 = yhat - lo95_hw
        hi95 = yhat + hi95_hw
    ...
    choice = ModelChoice(family=best_family, details=details, cv_smape=float(candidates_summary[best_family]["cv_smape"]))
    report = {
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Fit best model per station, export denoised series and forecast intervals.")
    p.add_argument("--input", required=True, help="Path to CSV containing time series for one or many stations.")
    ...
    p.add_argument("--station-col", required=False, default=None, help="Station ID column; if omitted, all rows treated as one station.")
    p.add_argument("--horizon", type=int, required=True, help="Forecast horizon (number of steps).")
    p.add_argument("--outdir", required=True, help="Output directory for per-station artifacts.")
    p.add_argument("--freq", default="auto", help="Pandas freq string (e.g., H, D, W, M) or 'auto'.")
    p.add_argument("--seasonal-periods", default="auto", help="Integer seasonal period or 'auto'.")
    p.add_argument("--impute", choices=["none", "ffill", "bfill", "linear"], default="linear", help="Gap imputation method after aligning to frequency.")
    p.add_argument("--n-splits", type=int, default=None, help="Rolling CV splits (default: auto).")
    p.add_argument("--initial-window", type=int, default=None, help="Initial training window for CV (default: auto).")
    return p.parse_args()
...
def main():
    ...
    for sid, g in (groups if isinstance(groups, list) else groups):
        try:
            ...
            print(f"[OK] {sid}: {artifacts.chosen_model.family} (CV sMAPE={artifacts.chosen_model.cv_smape:.2f})")
...
### Notes & practical defaults

* **Seasonality & frequency**: the script tries hard to infer both. For squeaky‑clean data, pass `--freq` and `--seasonal-periods` explicitly for best results.
* **Intervals**:

  * **SARIMA** uses model‑based intervals (exact via `conf_int`).
  * **ETS/SNaive** use a **conservative Normal approximation** from in‑sample residuals with √h scaling (suitable default; swap to bootstrap later if you want).
* **Model selection**: rolling origin with sMAPE; defaults aim to need **no extra flags**.
* **Gaps**: by default we **linearly interpolate** after aligning to frequency. Use `--impute none` if you prefer to fail fast on missingness.

---

### Extending (if you later want to)

* Add **residual bootstrap** intervals for ETS (simulate paths and take empirical quantiles).
* Include **Theta** or **STL+SARIMA** hybrids as candidates.
* Add **per‑station overrides** for `seasonal_periods` via a small YAML/JSON map.

If you’d like, I can also tailor the CLI defaults to your dataset’s frequency (e.g., hourly) and prune the SARIMA grid for faster runs.
