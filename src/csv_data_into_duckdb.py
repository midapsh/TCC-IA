...
logging.basicConfig(
    filename=CONFIGS.LOG_FOLDER.joinpath("csv_data_into_duckdb.log"),
    level=logging.DEBUG,
    format=(
...
def get_all_files() -> list[Mapper]:
    ...
    with duckdb.connect(database=str(CONFIGS.DUCKDB_DATABASE_URI)) as conn:
...
def save_data_batch(
    ...
    """
    if not list_station_timeseries:
        return

    processes = max((cpu_count() or 4) - 1, 2)
    with duckdb.connect(
        database=str(CONFIGS.DUCKDB_DATABASE_URI),
        config={
            "threads": processes,
            "memory_limit": "16GB",
        },
        ...
        conn.execute(
            "PRAGMA checkpoint_threshold='8GB';"
        )  # fewer checkpoints during big loads
